...
export default class Accordion extends React.Component<IAccordionProps, IAccordionState> {
  private cdns = [   "https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/4.3.1/css/bootstrap.min.css",
  ...
  constructor(props: IAccordionProps) {
    ...
    // DONT DO IT COMPA
    // SPComponentLoader.loadCss('1');
    // SPComponentLoader.loadCss('2');
    this.cdns.forEach(style => SPComponentLoader.loadCss(style));
  }
                ...
                >
                  {this.state.heroes.map((heroe, index) => (
                    <Draggable key={heroe.id} draggableId={heroe.id + 1} index={index}>
                      {(provided) => (
                        <div className="col-12 roll-in-left"
